...
{
    public class PorkFreeRule : Rule
    {
        ...
        {
            When()
                .Match<Customer>(() => customer, customer => customer.IsPorkFree)
                .Query(() => orderList, x => x
                    .Match<Order>(
                        o => o.CustomerId == customer.CustomerId,
                        o => o.IsValid,
                        o => o.Entrees.Where(e => e.ForPorkFree == false).Any() ||
                             o.Combos.Where(c => c.ForPorkFree == false).Any())
                    .Collect());
            Then()
        ...
        private static void InvalidateOrders(IEnumerable<Order> orderList, Customer customer)
        {
            ...
            {
                order.IsValid = false;
                Console.WriteLine("An order was found to be invalid on the basis of your aversion to pork");
