...
namespace FastFoodValidation.Rules
{
    public class VeganRule : Rule
    {
        ...
        {
            When()
                .Match<Customer>(() => customer, customer => customer.IsVegan)
                .Query(() => orderList, x => x
                    .Match<Order>(
                        o => o.CustomerId == customer.CustomerId,
                        o => o.IsValid,
                        o => o.Entrees.Where(e => e.ForVegan== false).Any() ||
                             o.Drinks.Where(d => d.ForVegan == false).Any() ||
                             o.Sides.Where(s => s.ForVegan == false).Any() ||
                             o.Combos.Where(c => c.ForVegan == false).Any())
                    .Collect());
            Then()
        ...
        private static void InvalidateOrders(IEnumerable<Order> orderList, Customer customer)
        {
            ...
            {
                order.IsValid = false;
                Console.WriteLine("An order was found to be invalid on the basis of your veganism");
