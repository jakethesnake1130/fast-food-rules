...
{
    public class GlutenFreeRule : Rule
    {
        ...
        {
            When()
                .Match<Customer>(() => customer, customer => customer.IsGlutenFree)
                .Query(() => orderList, x => x
                    .Match<Order>(
                        o => o.CustomerId == customer.CustomerId,
                        o => o.IsValid,
                        o => o.Entrees.Where(e => e.ForGlutenFree == false).Any() ||
                             o.Sides.Where(s => s.ForGlutenFree == false).Any() ||
                             o.Combos.Where(c => c.ForGlutenFree == false).Any())
                    .Collect());
            Then()
        ...
        private static void InvalidateOrders(IEnumerable<Order> orderList, Customer customer)
        {
            ...
            {
                order.IsValid = false;
                Console.WriteLine("An order was found to be invalid on the basis of your aversion to gluten");
